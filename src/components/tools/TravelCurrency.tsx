...
export function TravelCurrency() {
  ...
  const calculate = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/travel-tools/currency-converter`, {
        method: 'POST',
      ...
      <div className="bg-white border-4 border-black p-8 rounded-2xl shadow-[8px_8px_0px_0px_#000]">
        ...
        {result && !isLoading && (
          <div className="mt-8 bg-black text-white border-4 border-black p-6 rounded-xl shadow-[4px_4px_0px_0px_#000] animate-in slide-in-from-bottom-4">
            ...
            {result.rate && (
              <div className="text-center text-gray-400 mt-2 font-bold">
                Rate: 1 {fromCurrency} = {result.rate} {toCurrency}
              </div>
            )}
          </div>
        )}

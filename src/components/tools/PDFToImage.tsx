...
import { FileText, Upload, Download, Loader2, Image as ImageIcon } from 'lucide-react';
...
export function PDFToImage() {
  ...
  const handleConvert = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/docuforge/pdf-to-image`, {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 bg-white border-4 border-black rounded-2xl shadow-[8px_8px_0px_0px_#000]">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h2 className="text-3xl font-black uppercase">PDF to Image</h2>
      </div>
      ...
      <div className="mb-8">
        <label className="font-black uppercase text-lg block mb-4">Output Format</label>
        <div className="flex gap-4">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="font-bold text-lg">JPEG</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <input 
              type="radio" 
      ...
      >
        {isProcessing ? (
          ...
          <><Download className="w-6 h-6" /> Extract Images (ZIP)</>
        )}
      </button>
    </div>
  );
...
export const pdfToImageInstructions = [
  "Upload the PDF file you want to convert into images.",
  "Select the desired output format (JPEG or PNG).",
  "Click 'Extract Images' to receive a ZIP file containing every page as a separate image."
];

...
export function PDFToExcel() {
  ...
  const handleConvert = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/docuforge/pdf-to-excel`, {
        method: 'POST',
        body: formData,
      });

      if (!response.ok) throw new Error('Failed to convert PDF to Excel');

      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'converted_document.xlsx';
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 bg-white border-4 border-black rounded-2xl shadow-[8px_8px_0px_0px_#000]">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h2 className="text-3xl font-black uppercase">PDF to Excel</h2>
      </div>
        ...
        {!file ? (
          <label className="flex flex-col items-center justify-center w-full h-48 border-4 border-dashed border-black rounded-2xl bg-gray-50 hover:bg-gray-100 cursor-pointer transition-colors">
            <div className="flex flex-col items-center justify-center pt-5 pb-6">
              ...
              <p className="mb-2 text-xl font-black uppercase">Click or drag PDF here</p>
              <p className="text-sm font-bold text-gray-500">Supports .PDF files</p>
            </div>
            <input 
              type="file" 
              className="hidden" 
      ...
      >
        {isProcessing ? (
          ...
          <><Download className="w-6 h-6" /> Convert to Excel</>
        )}
      </button>
    </div>
  );
...
export const pdfToExcelInstructions = [
  "Upload a PDF file.",
  "Click 'Convert to Excel'.",
  "The file will be securely processed and downloaded as an Excel file."
];

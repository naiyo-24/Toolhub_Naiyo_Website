...
export function WordToPDF() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (!selectedFile.name.endsWith('.doc') && !selectedFile.name.endsWith('.docx')) {
        setError('Please select a valid Microsoft Word file (.doc or .docx).');
        return;
      }
  ...
  const handleConvert = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/docuforge/word-to-pdf`, {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 bg-white border-4 border-black rounded-2xl shadow-[8px_8px_0px_0px_#000]">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h2 className="text-3xl font-black uppercase">Word to PDF</h2>
      </div>
        ...
        {!file ? (
          <label className="flex flex-col items-center justify-center w-full h-48 border-4 border-dashed border-black rounded-2xl bg-gray-50 hover:bg-gray-100 cursor-pointer transition-colors">
            <div className="flex flex-col items-center justify-center pt-5 pb-6">
              ...
              <p className="mb-2 text-xl font-black uppercase">Click or drag Word Document here</p>
              <p className="text-sm font-bold text-gray-500">Supports .DOC and .DOCX files</p>
            </div>
            <input 
              type="file" 
              className="hidden" 
...
export const wordToPDFInstructions = [
  "Upload a Microsoft Word document (.doc or .docx).",

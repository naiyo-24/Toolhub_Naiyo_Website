...
export function CSVToExcel() {
  ...
  const handleConvert = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/docuforge/csv-to-excel`, {
        method: 'POST',
        body: formData,
      });

      if (!response.ok) throw new Error('Failed to convert CSV to Excel');
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 bg-white border-4 border-black rounded-2xl shadow-[8px_8px_0px_0px_#000]">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h2 className="text-3xl font-black uppercase">CSV to Excel</h2>
      </div>
...
export const csvToExcelInstructions = [
  "Upload a CSV file.",

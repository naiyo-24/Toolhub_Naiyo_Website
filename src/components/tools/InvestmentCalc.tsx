...
export function InvestmentCalc() {
  const [lumpsum_amount, set_lumpsum_amount] = useState<string>('');
  ...
  const calculate = async () => {
    if (!lumpsum_amount || !expected_annual_return || !time_period_years) return;
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/finance-tools/investment-calculator`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          lumpsum_amount: Number(lumpsum_amount),
    ...
    <div className="flex flex-col gap-8 max-w-2xl mx-auto">
      <div className="bg-white border-4 border-black p-8 rounded-2xl shadow-[8px_8px_0px_0px_#000]">
        ...
        <div className="space-y-4 mb-6">
          ...
          <div>
            <label className="block text-lg font-black uppercase mb-2">Lumpsum Amount (₹)</label>
            <input 
              type="number" 
              value={lumpsum_amount}
              onChange={(e) => set_lumpsum_amount(e.target.value)}
        ...
        <button 
          onClick={calculate}
          disabled={isLoading || !lumpsum_amount || !expected_annual_return || !time_period_years}

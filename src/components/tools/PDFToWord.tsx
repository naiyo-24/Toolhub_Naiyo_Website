...
import { FileText, Upload, Download, Loader2, FileType } from 'lucide-react';
...
export function PDFToWord() {
  ...
  const handleConvert = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/docuforge/pdf-to-word`, {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 bg-white border-4 border-black rounded-2xl shadow-[8px_8px_0px_0px_#000]">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h2 className="text-3xl font-black uppercase">PDF to Word</h2>
      </div>
      ...
      >
        {isProcessing ? (
          ...
          <><Download className="w-6 h-6" /> Convert to Word (DOCX)</>
        )}
      </button>
    </div>
  );
...
export const pdfToWordInstructions = [
  "Upload the PDF file you want to convert.",
  "Click 'Convert to Word'.",
  "The file will be securely processed and downloaded as a .docx file."
];

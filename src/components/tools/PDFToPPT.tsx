...
export function PDFToPPT() {
  ...
  const handleConvert = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/docuforge/pdf-to-ppt`, {
        method: 'POST',
        body: formData,
      });

      if (!response.ok) throw new Error('Failed to convert PDF to PPT');

      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'converted_document.pptx';
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 bg-white border-4 border-black rounded-2xl shadow-[8px_8px_0px_0px_#000]">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h2 className="text-3xl font-black uppercase">PDF to PPT</h2>
      </div>
      ...
      >
        {isProcessing ? (
          ...
          <><Download className="w-6 h-6" /> Convert to PPT</>
        )}
      </button>
    </div>
  );
...
export const pdfToPPTInstructions = [
  "Upload a PDF file.",
  "Click 'Convert to PPT'.",
  "The file will be securely processed and downloaded as a PowerPoint file."
];

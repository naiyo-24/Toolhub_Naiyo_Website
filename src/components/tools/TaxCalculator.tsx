...
export function TaxCalculator() {
  const [annual_income, set_annual_income] = useState<string>('');
  const [result, setResult] = useState<any | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const calculate = async () => {
    if (!annual_income) return;
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/finance-tools/tax-calculator`, {
        method: 'POST',
    ...
    <div className="flex flex-col gap-8 max-w-2xl mx-auto">
      <div className="bg-white border-4 border-black p-8 rounded-2xl shadow-[8px_8px_0px_0px_#000]">
        ...
        <div className="space-y-4 mb-6">
          ...
          <div>
            <label className="block text-lg font-black uppercase mb-2">Annual Income (₹)</label>
            <input 
              type="number" 
              value={annual_income}
              onChange={(e) => set_annual_income(e.target.value)}
        ...
        <button 
          onClick={calculate}
          disabled={isLoading || !annual_income}

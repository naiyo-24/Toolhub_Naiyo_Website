...
import { Upload, FileText, Loader2, Download, Settings, Zap } from 'lucide-react';
...
export function CompressPDF() {
  ...
  const handleCompress = async () => {
    ...
    formData.append('extreme_mode', extremeMode.toString());
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/file-tools/pdf/compress`, {
        method: 'POST',
        body: formData,
      });
            ...
            >
              ...
              {file ? (
                <div className="flex flex-col items-center gap-4">
                  <FileText className="w-16 h-16 text-neo-blue" />
                  <div>
                    <p className="font-bold text-lg break-words">{file.name}</p>
                    <p className="text-gray-500 font-bold text-sm">Original: {(file.size / 1024 / 1024).toFixed(2)} MB</p>
                  </div>
                </div>
              ) : (
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 font-bold text-sm">Only PDF files supported</p>
                  </div>
                </div>
              )}
            </div>
              ...
              <div className="bg-gray-50 border-4 border-black p-6 rounded-xl shadow-[4px_4px_0px_0px_#000] space-y-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full border-4 border-black rounded-xl p-3 font-bold focus:outline-none focus:ring-4 focus:ring-neo-blue/50"
                  />
                </div>
                
                <div 
                  className={`border-4 border-black p-4 rounded-xl cursor-pointer transition-colors flex items-center justify-between ${extremeMode ? 'bg-neo-pink text-white' : 'bg-white'}`}
                  onClick={() => setExtremeMode(!extremeMode)}
                >
                  <div className="flex items-center gap-3">
                    <Zap className="w-6 h-6" />
                    <div>
                      <p className="font-black uppercase">Extreme Mode</p>
                      <p className={`text-xs font-bold ${extremeMode ? 'text-white/80' : 'text-gray-500'}`}>Rasterizes pages for maximum compression</p>
                    </div>
                  </div>
                  <div className={`w-6 h-6 border-4 border-black rounded-sm flex items-center justify-center ${extremeMode ? 'bg-black' : 'bg-white'}`}>
                    {extremeMode && <div className="w-2 h-2 bg-white" />}
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="bg-green-50 border-4 border-black p-6 rounded-xl text-center shadow-[4px_4px_0px_0px_#000]">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-white border-2 border-black p-3 rounded-lg">
                  ...
                  <p className="font-black text-xl">{(file!.size / 1024 / 1024).toFixed(2)} MB</p>
                </div>
                <div className="bg-neo-green border-2 border-black p-3 rounded-lg">
                  ...
                  <p className="font-black text-xl">{(compressedSize! / 1024 / 1024).toFixed(2)} MB</p>
                </div>
              </div>
              
              <div className="bg-neo-blue text-white border-2 border-black p-2 rounded-lg font-black inline-block px-4 shadow-[2px_2px_0px_0px_#000]">
...
export const compressPdfInstructions = [
  "Upload the PDF document you want to compress.",
  "Adjust the Quality slider. Lower quality shrinks the file size more.",
  "Turn on 'Extreme Mode' if your PDF contains a lot of uncompressed images.",

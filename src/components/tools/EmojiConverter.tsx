...
import { Smile, Loader2, Copy, Check } from 'lucide-react';
...
export function EmojiConverter() {
  ...
  const convertText = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/social-tools/emoji-converter`, {
        method: 'POST',
      ...
      <div className="bg-white border-4 border-black p-8 rounded-2xl shadow-[8px_8px_0px_0px_#000]">
        ...
        <div className="space-y-4 mb-6">
          ...
          <textarea 
            ...
            placeholder="Type some words like happy, pizza, or car..."
        ...
        <button 
          ...
          className="w-full bg-neo-pink border-4 border-black px-8 py-4 font-black uppercase text-xl rounded-xl shadow-[4px_4px_0px_0px_#000] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_0px_#000] transition-all disabled:opacity-50 flex items-center justify-center"
        >
          {isLoading ? <Loader2 className="w-6 h-6 animate-spin" /> : 'Add Emojis'}
        </button>
        ...
        {result && !isLoading && (
          <div className="mt-8 bg-black text-white border-4 border-black p-6 rounded-xl shadow-[4px_4px_0px_0px_#000] relative animate-in slide-in-from-bottom-4">
            ...
            <h4 className="text-2xl font-black uppercase mb-4 text-neo-pink">Result</h4>
            <div className="text-xl font-bold whitespace-pre-wrap pr-12">
              {result}
            </div>
          </div>
        )}

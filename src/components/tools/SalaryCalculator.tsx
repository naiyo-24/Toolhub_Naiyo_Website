...
export function SalaryCalculator() {
  const [ctc, set_ctc] = useState<string>('');
  const [basic_percentage, set_basic_percentage] = useState<string>('');
  const [hra_percentage, set_hra_percentage] = useState<string>('');
  const [standard_deduction, set_standard_deduction] = useState<string>('');
  const [result, setResult] = useState<any | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const calculate = async () => {
    if (!ctc || !basic_percentage || !hra_percentage || !standard_deduction) return;
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/finance-tools/salary-calculator`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ctc: Number(ctc),
          basic_percentage: Number(basic_percentage),
          hra_percentage: Number(hra_percentage),
          standard_deduction: Number(standard_deduction)
        }),
      });
    ...
    <div className="flex flex-col gap-8 max-w-2xl mx-auto">
      <div className="bg-white border-4 border-black p-8 rounded-2xl shadow-[8px_8px_0px_0px_#000]">
        ...
        <div className="space-y-4 mb-6">
          ...
          <div>
            <label className="block text-lg font-black uppercase mb-2">CTC (Annual) (₹)</label>
            <input 
              type="number" 
              value={ctc}
          ...
          <div>
            <label className="block text-lg font-black uppercase mb-2">Basic % of CTC</label>
            <input 
              type="number" 
              value={basic_percentage}
              onChange={(e) => set_basic_percentage(e.target.value)}
              placeholder="e.g. 50"
              className="w-full bg-gray-50 border-4 border-black p-4 text-xl font-bold rounded-xl shadow-[4px_4px_0px_0px_#000] focus:outline-none focus:bg-white focus:translate-x-[4px] focus:translate-y-[4px] focus:shadow-none transition-all"
            />
          </div>
          <div>
            <label className="block text-lg font-black uppercase mb-2">HRA % of Basic</label>
            <input 
              type="number" 
              value={hra_percentage}
              onChange={(e) => set_hra_percentage(e.target.value)}
              placeholder="e.g. 50"
              className="w-full bg-gray-50 border-4 border-black p-4 text-xl font-bold rounded-xl shadow-[4px_4px_0px_0px_#000] focus:outline-none focus:bg-white focus:translate-x-[4px] focus:translate-y-[4px] focus:shadow-none transition-all"
            />
          </div>
          <div>
            <label className="block text-lg font-black uppercase mb-2">Standard Deduction (₹)</label>
            <input 
              type="number" 
              value={standard_deduction}
              onChange={(e) => set_standard_deduction(e.target.value)}
        ...
        <button 
          onClick={calculate}
          disabled={isLoading || !ctc || !basic_percentage || !hra_percentage || !standard_deduction}

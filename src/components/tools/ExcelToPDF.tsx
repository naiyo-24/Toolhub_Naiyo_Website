...
import { FileText, Upload, Download, Loader2, FileSpreadsheet } from 'lucide-react';
...
export function ExcelToPDF() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (!selectedFile.name.endsWith('.xls') && !selectedFile.name.endsWith('.xlsx')) {
        setError('Please select a valid Microsoft Excel file (.xls or .xlsx).');
        return;
      }
  ...
  const handleConvert = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/docuforge/excel-to-pdf`, {
        method: 'POST',
        body: formData,
      });

      if (!response.ok) throw new Error('Failed to convert Excel to PDF');

      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'converted_spreadsheet.pdf';
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 bg-white border-4 border-black rounded-2xl shadow-[8px_8px_0px_0px_#000]">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h2 className="text-3xl font-black uppercase">Excel to PDF</h2>
      </div>
        ...
        {!file ? (
          <label className="flex flex-col items-center justify-center w-full h-48 border-4 border-dashed border-black rounded-2xl bg-gray-50 hover:bg-gray-100 cursor-pointer transition-colors">
            <div className="flex flex-col items-center justify-center pt-5 pb-6">
              ...
              <p className="mb-2 text-xl font-black uppercase">Click or drag Excel file here</p>
          ...
          <div className="flex items-center justify-between p-6 bg-neo-green border-4 border-black rounded-xl shadow-[4px_4px_0px_0px_#000]">
...
export const excelToPDFInstructions = [
  "Upload an Excel spreadsheet (.xls or .xlsx).",
  "Click 'Convert to PDF'.",
  "The file will be securely processed and downloaded as a PDF."
];
